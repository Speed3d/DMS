...
public enum OwnerType
{
    ...
    /// <summary>كشف رواتب شهر — **لسجلّ الإصدارات وحده** لا للمرفقات (ADR-026).</summary>
    /// <remarks>
    /// يُستعمل في <c>DocumentVersion.DocType</c> لحفظ لقطةٍ من الشهر قبل كل تعديلٍ بعد
    /// التسديد. ولا مرفقات على الشهر نفسه — الإيصال الموقَّع يخصّ **موظفاً بعينه** فيُعلَّق
    /// على <see cref="PayrollEntry"/>.
    /// </remarks>
    PayrollPeriod = 4,

    /// <summary>سطر راتب موظف في شهر — **لإيصال الاستلام الموقَّع** (بلاغ المالك ٦).</summary>
    /// <remarks>
    /// الإيصال يُوقّعه موظفٌ واحد عن شهرٍ واحد، فمالكُه السطر لا الشهر ولا الموظف: تعليقه
...
public enum EmployeeChangeType
{
    ...
    /// <summary>فُكّ إسناده عن هذه الشركة (ADR-028) — قيمةٌ جديدة في عمودٍ قائم، بلا مهاجرة.</summary>
    UnlinkedFromCompany = 7,

    /// <summary>رُبطت بطاقته بحساب نظام (ADR-033) — قيمةٌ جديدة في عمودٍ قائم، بلا مهاجرة.</summary>
    /// <remarks>
    /// ⚠️ **يُسجَّل هنا لا في سجلّ التدقيق وحده**: الربط يفتح للمستخدم رؤية راتب صاحب هذه
    /// البطاقة، فمن يقرأ ملفّ الموظف بعد أشهر يجب أن يرى **متى فُتح** وبيد مَن.
    /// </remarks>
    UserLinked = 8,

    /// <summary>فُكّ ربط بطاقته عن حساب النظام (ADR-033).</summary>
    UserUnlinked = 9,

    /// <summary>بُتّ في حسم إجازةٍ من كشف شهر (ADR-036) — قيمةٌ جديدة في عمودٍ قائم.</summary>
    /// <remarks>
    /// ⚠️ **يُسجَّل في ملفّ الموظف لا في سجلّ التدقيق وحده**: الأثر **مالٌ يُنقص من راتبه**،
    /// فمن يقرأ ملفّه بعد أشهر يجب أن يرى **لماذا نقص راتب ذلك الشهر** ومَن قرّره.
    /// </remarks>
    LeaveDeductionSettled = 10,
}
...
/// <remarks>
/// ⚠️ **الترتيب ليس تدرّجاً**: القيم مخزَّنة في عمودٍ رقميّ، والمعنى في كلٍّ لا في ترتيبها.
/// </remarks>
public enum PayrollPaymentStatus
{
    /// <summary>لم يُصرف — **وللموظف المزدوج تعني «لم يُحسم أمره بعد»** (ADR-028).</summary>
    Unpaid             = 0,
    ...
    /// <summary>
    /// **قرارٌ صريح: يُصرف من هذه الشركة** رغم أنه يعمل في شركةٍ أخرى (ADR-028).
    /// </summary>
    /// <remarks>
    /// ⚠️ **لماذا حالةٌ رابعة ولا يكفي <see cref="Unpaid"/>؟** لأن التسديد صار ممنوعاً حتى
    /// يُحسم أمر كل موظفٍ مزدوج، و<c>Unpaid</c> **لا تفرّق بين «لم يُقرَّر» و«قُرِّر أن يُصرف
    /// من هنا»** — وهما نقيضان عند بوّابة التسديد. (نظير درس `EligibleDaysIsManual`:
    /// «قيمةٌ موجودة» ليست «قيمةً اختارها إنسان»، والفرق يحتاج علامةً صريحة.)
    ///
    /// وقيمةٌ جديدة في عمودٍ قائم ⇒ **بلا مهاجرة**، والصفوف القديمة تبقى على معناها.
    /// وعند التسديد تتحوّل إلى <see cref="PaidByThisCompany"/> كنظيرتها.
    /// </remarks>
    ConfirmedByThisCompany = 3,
}
