...
if ($EmployeeUser -and $EmployeePwd) {
    ...
    if ($empTok) {
        ...
        if ($finEmp.Status -eq 200) {
            ...
            # 🔄 **وضُيّق النشاط إلى السوبر أدمن وحده (قرار المالك 2026-08-12):** رئيس الشركة
            #    **محجوب أيضاً** رغم إعفائه من كل الأقسام — فالتقرير رقابةٌ عليه هو كذلك.
            #    ⚠️ ونبحث عن رئيسٍ حقيقي؛ وغيابُه يُعلَن ولا يُتجاهَل صامتاً.
            $pres = (Api GET "/users" $null $adminTok $cid).Body | Where-Object { $_.role -eq "President" } | Select-Object -First 1
            if ($pres) {
                Ok "وُجد رئيس شركة للاختبار: معرّفه $($pres.userId)"
                Skip "لا كلمة مرور له — حارس حجب الرئيس يُختبر في الواجهة (reports_access_test)"
            } else {
                Skip "لا رئيس شركة في النظام — حجبُه محروسٌ في اختبارات الواجهة"
            }
