...
public class AppDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<Employee>(e =>
        {
            ...
            // 🔐 **حسابٌ واحد لبطاقةٍ واحدة، في الاتجاهين** (ADR-033).
            //    بطاقتان لمستخدمٍ واحد ⇒ `/profile` لا يعرف أيّ راتبٍ راتبُه.
            //    مستخدمان لبطاقةٍ واحدة ⇒ أحدهما يقرأ راتب الآخر — وهي **تسريبةُ رواتب**
            //    لا مجرّد خطأ بيانات. والقاعدة هنا لا في الخدمة وحدها: سباقُ طلبين
            //    متزامنين يمرّ من فحص الخدمة ويقف عند الفهرس.
            e.HasIndex(x => x.UserId).IsUnique()
                .HasFilter("[UserId] IS NOT NULL AND [IsDeleted] = 0");

            // ⚠️ **بلا مفتاح أجنبيّ عمداً**: `User` و`Employee` كيانان لهما فلتران عامّان
            //    مختلفان، ومفتاحٌ أجنبيّ هنا يجعل حذف المستخدم يتعثّر أو يجرّ البطاقة معه.
            //    الربط **مرجعٌ ضعيف** يُفكّ يدوياً، وقراءتُه تتحمّل أن يكون الحساب معطّلاً.
