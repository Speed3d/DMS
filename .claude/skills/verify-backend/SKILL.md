...
**ستة عشر سكربتاً في `backend/e2e/`** (ومعها `bootstrap-e2e` أوّلاً) — آخرُ تشغيلٍ كامل **2026-09-25 على قاعدةٍ جديدة (`DmsE2E_Batch5`)، صفر فشل في كلٍّ — المجموع 902** (+1 للتهيئة).
...
2. **الترتيب إلزاميّ على القاعدة الجديدة** — السكربتات تفترض أن **أوّل شركتين أبجدياً** شركتاها:
   ...
   `gaps` ← `lockdown` ← `drafts` ← `password-change` ← `backup-restore` **أخيراً**
...
**المجموع (تشغيلٌ واحد 2026-09-25): 902** = 46 + 59 + 26 + 22 + 66 + 180 + 58 + 45 + 164 + 32 + 22 + 36 + 59 + 24 + 22 + 41 (+1 للتهيئة). ومع `-EmployeeUser` في `incoming` و`reports`: **923**. **والرقم نفسه في `status-report.md` §1 و§7 — يُعاد جمعُه لا يُورَّث.**
