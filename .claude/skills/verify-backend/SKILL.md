...
**المجموع: 91 تحقّقاً** عبر ثلاثة سكربتات — 68 وارد · 10 أقسام · 13 نسخ/استعادة. النجاح في كلٍّ = `فشل: 0` ورمز خروج 0.
...
يغطّي **68 تحقّقاً**: التسجيل والترقيم `PREFIX-IN-YEAR-#####` · طرق الاستلام (`Manual/Mail/Email` ورفض غيرها) · مصفوفة انتقالات الحالة (المسموح والمرفوض) · الملاحظة الإلزامية عند «تم الرد» يدوياً · الإحالة · الأرشفة ومنع التعديل بعدها · الربط/فك الربط مع الصادر والربط العكسي ومنع الربط المزدوج · سجل الحركة بأسماء المنفّذين · البحث بكل المعاملات · المرفقات · صلاحيات الموظف (403/404).
...
10 تحقّقات (ADR-015): إنشاء قسمين وموظفين · **موظف يرى الكتاب المُحال لقسمه وإن لم يستلمه** · عزل بين الأقسام (404) · القوائم مفلترة بالقسم · صلاحية `CanManageIncoming` تتيح تغيير الحالة · موظف بلا صلاحية يُمنع (403).
⚠️ يُنشئ موظفَي اختبار (`emp_fin`, `emp_leg`) وأقساماً — بيئة تطوير فقط.

## 3) النسخ الاحتياطي والاستعادة — سكربت جاهز
