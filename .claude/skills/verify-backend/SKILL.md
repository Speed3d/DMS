...
**تسعة عشر سكربتاً في `backend/e2e/`** (ومعها `bootstrap-e2e` أوّلاً) — آخرُ تشغيلٍ كامل **2026-09-25 على قاعدةٍ جديدة (`DmsE2E_Batch8`)، صفر فشل في كلٍّ — المجموع 981** (+1 للتهيئة).
...
2. **الترتيب إلزاميّ على القاعدة الجديدة** — السكربتات تفترض أن **أوّل شركتين أبجدياً** شركتاها:
   ...
   `gaps` ← `version` ← `outgoing-log` ← `lockdown` ← `drafts` ← `password-change` ← `backup-upload` ← `backup-restore` **أخيراً**
...
.\e2e\outgoing-log-e2e.ps1   -AdminPwd $P -Base $B -Db $DB     # ADR-056: سجلّ حركة الصادر · صورة السوبر أدمن
...
| `outgoing-log` | **37** | 2026-09-25 | **ADR-056**: الإنشاء والتعديل (بأسماء الحقول) والاعتماد والتعديل بعده والربط وفكّه والحذف **مسجَّلة** · المحاولة المرفوضة لا تُسجَّل · **القارئ 403** · رقم الوارد لمن يراه وحده · صورة السوبر أدمن على حسابه |
...
**المجموع (تشغيلٌ واحد 2026-09-25 على `DmsE2E_Batch8`): 981** = 46 + 59 + 26 + 22 + 66 + 180 + 58 + 45 + 164 + 32 + 22 + 36 + 12 + 37 + 59 + 24 + 22 + 30 + 41 (+1 للتهيئة). ومع `-EmployeeUser` في `incoming` و`reports`: **1002**. **والرقم نفسه في `status-report.md` §1 و§7 — يُعاد جمعُه لا يُورَّث.**
