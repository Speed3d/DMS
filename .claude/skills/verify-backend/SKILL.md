...
**خمسة عشر سكربتاً في `backend/e2e/`** (ومعها `bootstrap-e2e` أوّلاً) — آخرُ تشغيلٍ كامل **2026-09-24 على قاعدةٍ جديدة (`DmsE2E_Batch4`)، صفر فشل في كلٍّ — المجموع 862** (+1 للتهيئة).
...
2. **الترتيب إلزاميّ على القاعدة الجديدة** — السكربتات تفترض أن **أوّل شركتين أبجدياً** شركتاها:
   ...
   `lockdown` ← `drafts` ← `password-change` ← `backup-restore` **أخيراً**
...
|---|---|---|---|
| `bootstrap` | **1** | 2026-09-24 | 🔴 **أوّلاً دائماً (ADR-052)**: يُخرج المدير المبذور من «يجب التغيير» بكلمته نفسها |
| `review-fixes` | **42** | 2026-09-24 | **ADR-048**: تعديل المدير/الرئيس لا يُسقط إسناداً ولا يصفّر أقساماً · المستمسكات والإيصالات بعلَم الكتابة · حذف الشركة بلا يتيمٍ في 24 جدولاً (يقرأ القاعدة) — وحذفُها **عمليةٌ خلفية** (ADR-049) |
| `isolation` | **59** | 2026-09-24 | العزل **بالمعرّف** عبر الوحدات · **12 اعتماداً متزامناً** بأرقامٍ متمايزة · عزل الإشعارات (ADR-046) · **التعطيل ثم الحذف** (ADR-047) خلفياً |
| `multi-company` | **26** | 2026-09-24 | موظفٌ في شركتين بصلاحياتٍ وأقسامٍ مختلفة (ADR-017) — نفس التوكن يُحجب في شركةٍ ويمرّ في أخرى |
| `departments` | **22** | 2026-09-24 | الإحالة **متعددة وتراكمية** · مَن أحال يبقى يرى · يُنشئ `emp_fin`/`emp_leg` |
| `incoming` | **66** (77 مع `-EmployeeUser`) | 2026-09-24 (66) · 2026-08-11 (77) | دورة الوارد كاملة + رؤية الصادر بالقسم (ADR-030) |
| `hr` | **180** | 2026-09-24 | الرواتب والإجازات والمستمسكات وبوّابة الحسم وتعديل المُسدَّد |
| `profile` | **58** | 2026-09-24 | البروفايل (ADR-033/035): يرى راتبه وحده · صورته · إجازته الذاتية |
| `reports` | **45** (55 مع `-EmployeeUser`) | 2026-09-24 (45) · 2026-08-11 (55) | النشاط والتفصيليان بحدٍّ مزدوج — 🟢 **يقرأ ولا يكتب** |
| `tasks` | **164** | 2026-09-24 | وحدة المهام كاملةً + الخدمة الخلفية (لا تُعيد إشعاراً ولا تكاثر) |
| `case-files` | **32** | 2026-09-24 | المعاملات والربط كثيرٌ إلى كثير (ADR-045) |
| `verify` | **22** | 2026-09-24 | صفحة التحقق العامّة `/v/{token}` (ADR-043/044) |
| `lockdown` | **59** | 2026-09-24 | **ADR-050**: السوبر أدمن وحده يتصفّح · الدخول 503 **بلا عدّ** والخاطئ يُحسب · التجديد بلا تدوير · `/v/*` يعمل · الشريط للمصادَق · **الاستعادة لا تُنهي الإيقاف** · أمر الطوارئ على السيرفر يسري والخادم يعمل (يقرأ القاعدة) |
...
| `password-change` | **22** | 2026-09-24 | **G19 (ADR-052)**: رمزُ الكلمة المؤقتة 403 عدا التغيير · الجديدة تختلف · التغيير يعيد رمزاً ويُبطل تجديد الأجهزة الأخرى · إعادةُ تعيين المدير تُنهي الجلسات |
| `backup-restore` | **41** | 2026-09-24 | نسخ ← حذف ← **استعادة** ← الكتاب يعود · المرآة · **البدء يردّ 202 فوراً** · **التنزيل تدفّقٌ كامل بالبايت** · عمليةٌ مجهولة 404 (ADR-049) |

**المجموع (تشغيلٌ واحد 2026-09-24): 862** = 42 + 59 + 26 + 22 + 66 + 180 + 58 + 45 + 164 + 32 + 22 + 59 + 24 + 22 + 41 (+1 للتهيئة). ومع `-EmployeeUser` في `incoming` و`reports`: **883**. **والرقم نفسه في `status-report.md` §1 و§7 — يُعاد جمعُه لا يُورَّث.**
