...
| POST | `/{id}/status` | تغيير الحالة `{status,note}` — يخضع لمصفوفة الانتقالات (ADR-013). **الأرشفة للمدير فأعلى دائماً.** الموظف/القارئ: `New→InReview` فقط، **إلا بصلاحية `CanManageIncoming`** فيدير كل الانتقالات عدا الأرشفة (ADR-015). الملاحظة **إلزامية** عند `Replied` يدوياً بلا ربط بصادر |
| POST | `/{id}/forward` | إحالة لقسم `{departmentId,note}` — للحالتين `New`/`InReview` فقط، وتنقل `New` تلقائياً إلى `InReview`. يتحقق أن القسم موجود ونشط وضمن نفس الشركة. **القارئ ممنوع (403)**. الكتاب يظهر بعدها لموظفي القسم |
