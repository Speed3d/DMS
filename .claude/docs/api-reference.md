...
| PUT | `/periods/{year}/{month}` | إعدادات الفترة (سعر الصرف · أيام العمل) — **يُعيد حساب الكشف كله**. يتطلّب `rowVersion` |
| PUT | `/periods/{year}/{month}/entries` | حفظ السطور دفعةً. ⚠️ **بلا صافٍ** — الخادم يحسبه ويتجاهل ما يرسله العميل. يتطلّب `rowVersion` |
