...
| POST | `/{id}/forward` | ⚠️ يُرجِع **204 بلا جسم** إن خرج الكتاب من قسم المُحيل (الوضع الطبيعي حين يُحيل موظفُ قسمٍ كتابَه لقسم آخر). إحالة لقسم `{departmentId,note}` — للحالتين `New`/`InReview` فقط، وتنقل `New` تلقائياً إلى `InReview`. يتحقق أن القسم موجود ونشط وضمن نفس الشركة. **القارئ ممنوع (403)**. الكتاب يظهر بعدها لموظفي القسم |
