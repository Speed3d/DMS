...
| POST | `/{id}/forward` | إحالة لقسم `{departmentId,note}` — للحالتين `New`/`InReview` فقط، وتنقل `New` تلقائياً إلى `InReview`. يتحقق أن القسم موجود ونشط وضمن نفس الشركة. الكتاب يظهر بعدها لموظفي القسم |
