...
- **فهرس فريد على `(IncomingId, OutgoingId)`** — الزوج نفسه لا يُربط مرّتين. والخدمة تفحص
  الوجود مسبقاً لتعطي **409 عربية**، والفهرس حارسُ التسابق فوقه.
- 🔴 **حلّ محلّ عمودَين مفردين** (`IncomingBook.ReplyOutgoingId` · `OutgoingBook.ReplyToIncomingId`)
  **أُسقطا في المهاجرة بعد نقل بياناتهما من الاتجاهين معاً**. والسبب أن العلاقة صارت **كثيراً
  إلى كثير**: صادرٌ واحد يُجيب عدّة واردات، وواردٌ يُجاب بردٍّ أوّليّ ثم نهائيّ.
- ⚠️ **والعمودان كانا يتباعدان فعلاً**: حذفُ الوارد يُفرغ جهة الصادر ويترك جهته، وحذفُ
  الصادر **لا يُفرغ شيئاً**. ولهذا يقرأ النقلُ الجهتين ويستبعد الأزواج المتباينة الشركة.
- **`CompanyId` منسوخٌ بفلترٍ عام** خلافاً لـ`IncomingAssignment` (التي بلا `CompanyId` لأنها
  تُقرأ عبر أبيها وحده): هذا الجدول **يُستعلَم عنه مباشرةً** من جهة الصادر.
- **`LinkedByUserId` لاغٍ وبلا مفتاح أجنبيّ** — الروابط المنقولة بلا فاعلٍ تاريخيّ، و`0`
  يخرق المفتاح فتفشل المهاجرة (نمط `Employee.UserId`).
- ⚠️ **بلا حذفٍ ناعم**: يُحذف فعلياً في `SoftDeleteAsync` على **الطرفين**، لأن صفّاً يشير
  إلى كتابٍ محذوف **يختفي من كل استعلام** فيبقى الطرف الآخر «تم الرد» بردٍّ لا يُرى.
- 🔴 **والتعاقبان آمنان بعد إسقاط المفتاحين القديمين فقط** — مع بقائهما يصير من
  `OutgoingBooks` إلى هذا الجدول **مساران** فيرفضه SQL Server بخطأ **1785**.
