...
class SessionState {
  ...
  /// صلاحية **إدارة مهام الآخرين** — إسناداً وتعديلاً وإعادةَ فتح (ADR-037).
  ///
  /// ⚠️ ليست شرطاً لرؤية الوحدة: بدونها يُنشئ الموظف مهامّاً **لنفسه** ويحدّث مهامّ قسمه.
  bool get canManageTasks => auth?.canManageTasksIn(effectiveCompanyId) ?? false;

  /// هل يظهر قسم المهام؟ (القسم **مع** الدور — مرآةٌ لـ`[RequireGrantedModule]`)
  ///
  /// 🔐 **القارئ محجوبٌ ولو مُنح القسم** (قرار المالك): دورُه اطّلاعٌ على الوثائق، والمهمة
  /// **تكليفٌ يُنفَّذ ويُحدَّث** لا وثيقةٌ تُقرأ. والحدّان معاً لا أحدهما — بندٌ في الشريط
  /// يقود إلى شاشة تردّ 403 أسوأ من إخفائه.
  bool get canSeeTasks => hasModule('Tasks') && _aboveReader;
  ...
  /// تقرير النشاط: **السوبر أدمن وحده** (ADR-031، ضُيّق بقرار المالك 2026-08-12).
  ///
  /// 🔄 كان «رئيس الشركة فأعلى»، فصار السوبر أدمن وحده: التقرير **رقابةٌ على المستخدمين
  /// أنفسهم بمن فيهم الرئيس** — فمن يُراقَب لا يملك أداة المراقبة.
  /// مرآةُ `[Authorize(Roles = "SuperAdmin")]` على نقاط النشاط الأربع و`/api/audit`.
  ///
  /// ⚠️ ولا يُشترط القسم هنا لأن السوبر أدمن **معفى من قيد الأقسام أصلاً** — واشتراطُه
  /// يوهم بحدٍّ ثانٍ لا وجود له.
  bool get canSeeActivityReport => auth?.role == 'SuperAdmin';
